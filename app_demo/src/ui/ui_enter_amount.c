...
void create_numpad(lv_obj_t *parent) {    
    ...
    lv_obj_set_style_bg_color(numpad,lv_palette_lighten(LV_PALETTE_GREY, 3), 0);  // Light grey background
    ...
    // Number keys 1-9
    const char *nums[] = {"1", "2", "3", "4", "5", "6", "7", "8", "9", "Clear", "0", "Backspace"};
    for(int i = 0; i < 12; i++) {
        ...
        { 
            ...
            lv_obj_set_style_text_color(label, lv_color_hex(0xED7D63), 0);
        }else if(i == 11){
            lv_obj_set_style_text_font(label, &ali_middle_24, 0);  //28
            lv_obj_set_style_text_color(label, lv_color_hex(0xEDD849), 0);
        }else{
    ...
    lv_obj_set_style_bg_color(btn, lv_color_hex(0x9BD6B1), 0);
